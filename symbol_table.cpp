...
public:
  ...
  Node (string lexeme, string token, string value, int lineNo, char char_start_num, int length) {
    this->lexeme = lexeme;
    this->token = token;
    this->value = value;
    this->lineNo = lineNo;
...
public:
  ...
  bool insert(string lexeme, string token, string value, int lineNo, char char_start_num, int length);
  string find (string lexeme);
};

// Find the lexeme in the symbol table
